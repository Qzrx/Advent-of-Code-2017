...
input_matrix = '''4347	3350	196	162	233	4932	4419	3485	4509	4287	4433	4033	207	3682	2193	4223
...
def formatter(input_value):
    return([x.split('\t') for x in input_value.split('\n')])

def checksum(input_spreadsheet):
	row_checksums = [int(0)]
	for row in input_spreadsheet:
		row = [int(val) for val in row]
		L = len(row)
		for idx in xrange(0, L):
			comparison_value = row[idx]
			for comparison_idx in xrange(idx+1, L):
				last = row[comparison_idx]
				print('Comparing: ' + str(comparison_value) + ', ' + str(last))
				if max(comparison_value, last) % min(comparison_value, last) == 0:
					print('Success with ' + str(comparison_value) + ', ' + str(last))
					row_checksums.append(max(comparison_value, last) / min(comparison_value, last))
					print('Row checksums are now ' + str(row_checksums))
					break
	return(sum(row_checksums))

print(checksum(formatter(test_matrix)))
print(checksum(formatter(input_matrix)))