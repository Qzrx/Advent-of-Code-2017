# Day 8 - I Heard You Like Registers-1.py
# TODOS:
#   * What should the default behavior for registers be?
#   * Add in the test stack to parse/execute
#   * Change that ugly nest of `if` statements to use a dict
import pprint

test_stack = '''b inc 5 if a > 1
a inc 1 if b < 5
c dec -10 if a >= 1
c inc -20 if c == 10'''

fancy_stack = '''utc dec -736 if p > -7
tn inc -876 if qlm == 4
uz dec 294 if l < 10
a inc -904 if me >= -7
tn inc 622 if ppl <= 6
...
def parser(instruction):
    ...
    amount = int(instruction_parts[2])
    conditional = instruction_parts[3:][1:]  # tosses out the 'if' part, because that's a given
    conditional_register = conditional[0]
    conditional_operator = conditional[1]
    conditional_value = int(conditional[2])
    return(register, instruction, amount, conditional_register, conditional_operator, conditional_value)



def machine(stack):
    register_names = set([instruction.split(' ')[0] for instruction in stack.split('\n')])
    registers = {}

    # Build our set of registers
    for register in register_names:
        registers[register] = Register(register)

    # Parse the stack
    for instruction in stack.split('\n'):
        parsed_parts = parser(instruction)
        
        operational_register = parsed_parts[0]
        operational_instruction = parsed_parts[1]
        operational_amount = parsed_parts[2]
        conditional_register = parsed_parts[3]
        conditional_operator = parsed_parts[4]
        conditional_value = parsed_parts[5]

        if conditional_operator == '>':
            if registers[conditional_register].value > conditional_value:
                registers[operational_register].operate(operational_instruction, operational_amount)
        if conditional_operator == '>=':
            if registers[conditional_register].value >= conditional_value:
                registers[operational_register].operate(operational_instruction, operational_amount)
        if conditional_operator == '<':
