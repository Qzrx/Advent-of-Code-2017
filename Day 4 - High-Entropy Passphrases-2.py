input_tests = '''abcde fghij
abcde xyz ecdab
a ab abc abd abf abj
iiii oiii ooii oooi oooo
oiii ioii iioi iiio'''

input_values = '''bdwdjjo avricm cjbmj ran lmfsom ivsof
...
def validate_passphrase(passphrase):
	...
	L = len(passphrase)
	print('Passphrase tokens: ' + str(passphrase))
	for idx in xrange(0, L-1):
		word = list(passphrase[idx])
		word.sort()
		for comparison_idx in xrange(idx+1, L):
			comparison_word = list(passphrase[comparison_idx])
			comparison_word.sort()
			print('Comparing: ' + str(word) + ', ' + str(comparison_word))
			if word == comparison_word:
				return(False)
	return(True)

print([validate_passphrase(passphrase) for passphrase in input_tests.split('\n')])
print(sum([validate_passphrase(passphrase) for passphrase in input_tests.split('\n')]))

print([validate_passphrase(passphrase) for passphrase in input_values.split('\n')])
print(sum([validate_passphrase(passphrase) for passphrase in input_values.split('\n')]))